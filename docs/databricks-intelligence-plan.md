...
| 4 | Workspace half verified live; browser half not | Deployed and exercised on 2026-09-02: Lakebase, both synced tables, the App and its resource binding, the serving-sync task, and the least-privilege grants. One job run carried Gold through to Lakebase and the App served those rows to an authenticated caller in the browser's own schema; an unauthenticated request was refused. Not yet verified, and all of it needs the Worker deployed to Cloudflare rather than more Databricks work: the browser-through-Worker path, an invalid sync token, an unauthorized service principal, the 35-minute freshness run timed from a real browser event, and the deletion round trip. |
