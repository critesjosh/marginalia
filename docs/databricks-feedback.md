...
- Workaround or follow-up: a `serving_grants` task now runs after the syncs and grants over the tables that exist rather than the ones that existed, then asks `has_table_privilege` whether each grant actually took and fails if one did not. It closes the window rather than removing it: a table created by a deploy is granted by the next run of the job. The general shape is worth keeping: a resource created by one identity for another to read needs the grant re-applied when the resource is re-created, and a status field reporting the resource is healthy is not reporting that.
...
- Workaround or follow-up: submit the affected task as a one-off run against the file the deploy had already uploaded (`jobs/runs/submit` pointing at the workspace path), which is how the grant task was run and verified. Upgrading the CLI is the real fix; v1.14.1 sends a mask its own control plane rejects, and until it is upgraded the job definitions in this bundle cannot be updated at all.
