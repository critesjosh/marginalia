...
A relevance floor keeps ordinary English out. A chunk is quoted only when the terms it shares with the query are distinctive enough: measured against this book, "Bulkington" scores 9.1 and "Queequeg" 2.9 against a bar of 2.2, while "whale" reaches 1.6 and "day" plus "men" 1.9. A thematic question that shares no distinctive term with the passages that would answer it therefore gets no excerpts at all, which is the honest result — that question needs a model, not a keyword search.

Two further rules exist because sending a passage is not free: it competes for the model's attention with the one the reader actually highlighted. Excerpts must score within 45% of the best match, so a query with one real hit sends one rather than one and three passable; and only one excerpt per chapter, since consecutive chunks of a chapter score alike and read as the same passage twice. The vocabulary of asking about books — *mentioned*, *describes*, *chapter* — is stoplisted, because Melville writes "mentioned" far less often than he writes his own harpooneer, and without that "where has Queequeg been mentioned?" ranks on the wrong word. A question outweighs the highlighted passage, so a chat that drifts follows the reader rather than staying pinned to where it started.
